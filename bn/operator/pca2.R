...
#   print("shinyServerShowResults")
#   listResult = internal_dataFrameOperator(data, properties=properties, folder=folder)
#   print("shinyServerShowResults")
#   aRunDataList = listResult$showResults
  #   mapProperties = client$getPropertiesAsMap()
  
    aRunFile = paste(folder, '\\runData.rda', sep = '')
    load(aRunFile)
  aPca = aRunDataList$aPca    
  sAnnotation = aRunDataList$sampleAnnotation
  cAnnotation = aRunDataList$colorAnnotation
  varNames = aRunDataList$varNames
  
  nComp = getPropertyValue(properties, "Number of Components", TRUE)
  #   nComp = mapProperties[["Number of Components"]]
  
  px = paste("PC", 1:nComp, sep = "")
  py = px[c(2,1,3:nComp)]
